...
# sciagamy listę tabel z pliku z ustawieniami
LISTA_TABEL = USTAWIENIA["NazwaTabeli"].tolist()
LISTA_TABEL = [i for i in LISTA_TABEL if str(i) != "nan"]
...
if pd.notna(USTAWIENIA.loc[0, "TabelaBezEM"]) is True:
    for i in WYCINEK["Kolumna 0"]:
        if i == USTAWIENIA.loc[0, "TabelaBezEM"]:
            ADRES = WYCINEK_INDEX.get_loc(i) + 3
...
# do edycji arkuszy
# żeby go skonfigurować musi zidentyfikować plik xls który będzie edytowany:
ADRES_EXCELA = '{}\\{}'.format(os.getcwd(), USTAWIENIA.loc[0, "NazwaPliku"])
...
for i in LISTA_DOLKOW:
    ...
    for tabela in LISTA_TABEL:
        ...
        WYCINEK = GLOWNA_BAZA.iloc[USTAWIENIA.iloc[X, 1]:USTAWIENIA.iloc[X, 2],
                                   NR_DOLKA]
    ...
    if pd.notna(USTAWIENIA.loc[0, "TabelaBezEM"]) is True:
        WYCINEK = GLOWNA_BAZA.iloc[USTAWIENIA.iloc[0, 4]:USTAWIENIA.iloc[0, 5],
                                   NR_DOLKA]
        WIERSZ_MACIERZY = WIERSZ_MACIERZY.append(WYCINEK, ignore_index=True)
...
for dolek in LISTA_DOLKOW:
    ...
    for i in LISTA_TABEL:
        ...
        FALA = GLOWNA_BAZA.iloc[USTAWIENIA.iloc[X, 1]:USTAWIENIA.iloc[X, 2], 1]
        DOLEK = GLOWNA_BAZA.iloc[USTAWIENIA.iloc[X, 1]:USTAWIENIA.iloc[X, 2],
                                 NR_DOLKA]
